...
def test_create_bundle_converts_iconset_to_icns(tmp_path, monkeypatch):
    binary = tmp_path / "rollshot-app"
    binary.write_bytes(b"fake-binary")
    iconset = tmp_path / "rollshot.iconset"
    iconset.mkdir()
    ...
    def fake_run(command):
        ...
        output.write_bytes(b"fake-icns")

    monkeypatch.setattr(
        macos_bundle.shutil,
        "which",
        lambda name: "/usr/bin/iconutil" if name == "iconutil" else None,
    )
    monkeypatch.setattr(macos_bundle, "run", fake_run)
    ...
    assert (app / "Contents" / "Resources" / "rollshot.icns").read_bytes() == b"fake-icns"
    assert commands == [
        [
            "/usr/bin/iconutil",
            "-c",
            "icns",
            "-o",
            str(app / "Contents" / "Resources" / "rollshot.icns"),
            str(iconset),
        ]
    ]
