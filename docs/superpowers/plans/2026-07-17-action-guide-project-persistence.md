...
- Crash leftovers are accepted, not corruption: a crash may orphan content-addressed PNGs under `assets/frames/` or leave `project.json.tmp-*` siblings. Load reads only `project.json` and manifest-referenced assets, so orphans are ignored; garbage collection is deferred to a later plan. Never add ad-hoc cleanup that deletes files, since writer locking does not exist until Plan 2.
...
- `Cargo.lock` — regenerated by the dependency additions; commit alongside the manifests.
...
Validation never inspects hashes, and `SnapshotFramePayload::Pixels` frames have no digest until save-time encoding — so `validate_common` takes a hash-free metadata view instead of `ProjectFrame`:
...
pub fn validate_manifest_structure(manifest: &ProjectManifestV1) -> Result<(), ProjectError> {
    ...
    let frames = manifest.frames.iter().map(FrameMeta::from_project_frame).collect::<Vec<_>>();
...
Add this transaction diagram to the `store.rs` module doc comment and keep it accurate as the code evolves — the two paths have different commit points:

```text
first Save / Save As                      existing Save
────────────────────                      ─────────────
snapshot                                  snapshot (base_revision = Some(n))
   │ build temp dir sibling (RAII guard)     │ preflight: read_manifest, compare revision
   │ materialize every asset                 ├─ mismatch ─▶ RevisionConflict (disk untouched)
   │ validate revision-1 manifest            │ materialize missing immutable assets
   │ write_manifest_atomic + publish/        │ re-read revision just before commit
   ▼                                         ├─ changed ──▶ RevisionConflict (disk untouched)
renameat_with(NOREPLACE) on the dir          ▼
   ├─ EEXIST ─▶ DestinationExists         write_manifest_atomic(revision n + 1):
   ▼                                        tmp ─▶ sync_all ─▶ same-dir rename ─▶ fsync dir
COMMIT = the directory rename             COMMIT = the manifest rename
```
...
```bash
rtk git add crates/rollshot-action/tests/project_persistence.rs
