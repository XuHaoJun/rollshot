...
**Architecture:** Move the placeholder Rust app into a Tauri `src-tauri` crate named `rollshot-app`, with a small React frontend in `crates/rollshot-app/src`. This keeps the same app-root shape as `learn-projects/snow-shot` and `learn-projects/tauri-template`: frontend files in app-root `src/`, native code in app-root `src-tauri/src/`; the outer `crates/` directory remains because this repository is a Rust workspace. Rust owns capture state and stores the latest full-resolution frame; the capture stream is constructed inside the worker thread so platform stream objects never need to cross thread boundaries. Region selection math lives in TypeScript as pure functions so HiDPI/source-pixel conversion is tested without a running portal.

**Tech Stack:** Rust 2021, Tauri v2, Node.js, pnpm, React, TypeScript, Vite, Vitest, shadcn/ui v4, Tailwind CSS v4, Lucide React, `rollshot-capture`, `image`, serde/serde_json.
...
- Use Node.js 24.15.0 through mise plus `pnpm`, matching `learn-projects/snow-shot`'s package-manager choice. The first implementation run should create `crates/rollshot-app/mise.toml` and `crates/rollshot-app/pnpm-lock.yaml`; do not create `package-lock.json`, `bun.lock`, or `yarn.lock`.
- Use `learn-projects/snow-shot` as the main app-shape reference, but replace Ant Design / `@ant-design/icons` with shadcn/ui v4, Tailwind CSS v4, and Lucide React as in `learn-projects/tauri-template`.
- Use Vite, not Rsbuild. `learn-projects/snow-shot` uses Rsbuild, but Rollshot's Plan 2 needs a small single-screen frontend and shadcn/ui's first-class Vite/Tailwind CSS v4 path is simpler than bringing in Rsbuild and TanStack Router.
- Do not use Corepack in this plan. `pnpm` is expected to be installed by the developer environment, for example via `mise use -g pnpm@latest`.
- Keep `crates/rollshot-app` as the Tauri app root. Inside that root, `src/` is the React frontend and `src-tauri/src/` is Rust. Moving the app to repository-root `src/` would fight the existing Rust workspace layout without simplifying the Tauri app.
...
- `crates/rollshot-app/package.json`
  - Node.js/pnpm frontend scripts and Tauri CLI entrypoint.

- `crates/rollshot-app/mise.toml`
  - App-local Node.js version pin for frontend tooling.

- `crates/rollshot-app/pnpm-lock.yaml`
  - Frontend lockfile created by `pnpm install`.

- `crates/rollshot-app/components.json`
...
- `crates/rollshot-app/vite.config.ts`
  - React/Vite/Vitest config with Tailwind CSS v4 plugin and `@` alias.

- `crates/rollshot-app/src/main.tsx`
  - React root mounting.

- `crates/rollshot-app/src/App.tsx`
  - Minimal interactive capture UI shell using shadcn `Button` and Lucide icons.

- `crates/rollshot-app/src/App.css`
  - Tailwind CSS v4 entrypoint, shadcn theme variables, and functional layout for preview, controls, and selection overlay.

- `crates/rollshot-app/src/components/ui/button.tsx`
  - shadcn/ui Button component generated by the shadcn CLI.

- `crates/rollshot-app/src/lib/utils.ts`
  - `cn()` helper for shadcn class merging.
...
- Create: `crates/rollshot-app/mise.toml`
- Create: `crates/rollshot-app/pnpm-lock.yaml`
- Create: `crates/rollshot-app/components.json`
...
- Create: `crates/rollshot-app/src/App.css`
- Create: `crates/rollshot-app/src/components/ui/button.tsx`
- Create: `crates/rollshot-app/src/lib/utils.ts`
...
- [ ] **Step 3: Pin the app Node.js runtime with mise**

Run:

```bash
rtk mise use --path crates/rollshot-app --pin node@24.15.0
```

Expected: `crates/rollshot-app/mise.toml` is created with:
...
- [ ] **Step 5: Create shadcn/ui and Tailwind CSS config files**
...
- [ ] **Step 6: Create the Vite entry files**
...
- [ ] **Step 7: Add the shadcn Button component**

Run:

```bash
rtk pnpm --dir crates/rollshot-app dlx shadcn@latest add button
```

Expected: `crates/rollshot-app/src/components/ui/button.tsx` is created and imports `cn` from `@/lib/utils`.

- [ ] **Step 8: Create the first React screen**
...
- [ ] **Step 9: Create the Tauri Rust manifest and config**
...
- [ ] **Step 10: Create the native entrypoint**
...
- [ ] **Step 11: Delete the placeholder Rust package**
...
Expected: `pnpm-lock.yaml` exists or is updated and dependencies install successfully. No `package-lock.json`, `bun.lock`, or `yarn.lock` is created.
...
```bash
rtk pnpm --dir crates/rollshot-app test -- --run src/region/geometry.test.ts
```
...
Append this CSS to `crates/rollshot-app/src/App.css`:
...
- snow-shot package-manager convention: Task 1 uses mise-pinned Node.js 24.15.0 with pinned `pnpm@10.8.1`, `pnpm-lock.yaml`, and Tauri `before*Command` values that call `pnpm`.
- Build-tool decision: Task 1 uses Vite rather than snow-shot's Rsbuild because the Plan 2 frontend is small and shadcn/ui's Vite/Tailwind CSS v4 path is the lower-risk scaffold.
- shadcn/Tailwind scaffold preference: Task 1 creates `components.json`, Tailwind CSS v4 Vite wiring, shadcn theme variables, `cn()`, and the shadcn `Button` component through `pnpm dlx shadcn@latest add button`.
- Ant Design replacement: Task 1 and Task 5 use shadcn `Button` and Lucide React icons; no Ant Design dependencies are introduced.
...
- The plan avoids placeholder markers and excludes Corepack, npm/Bun/Yarn lockfiles, Ant Design dependencies, speculative settings UI, updater, tray, Windows support, auto-scroll, TanStack Router, Rsbuild, and SharedBuffer work.
